...
{ 
		...
		else   
		{
				...
				//HCLK 不分频;APB1 为AHB4分频;APB2 为AHB2分频.	
				//实际上HSE时钟4-26MHz进来先过锁相PLLCLK变成系统时钟180MHz
				//系统时钟进入AHB后进行总线分频，分频后再变成外设时钟APB1和APB2
				RCC->CFGR |= (0<<4)|(5<<10)|(4<<13);
			
				RCC->CR&=~(1<<24);					//关闭主PLL
				//配置主PLL,PLL时钟源来自HSE
				//这里Pllp只能为2,4,6,8，与时钟树图上不对应，但是与寄存器PLLCFGR说明吻合
